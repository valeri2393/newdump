...
from util.config import build_config
from util.db import request_sql
from webapp.const import HEADER_FORMAT, SOURCE_FORMAT, CHART_FORMAT
from webapp.styles import set_style
from webapp.util import get_last_date, get_types, get_sql_list, cell_styling

# Устанавливаем правильный формат даты
DATE_FORMAT = "%d.%m.%Y"
...
resources = [data[0] for data in request_sql(
    conn, f"SELECT DISTINCT resource FROM cards WHERE type = '{type}'"
)]

# Проверка наличия ресурсов
if not resources:
    st.error("Нет доступных ресурсов для выбранного типа.")
    st.stop()

# Выбор ресурса
resource = st.sidebar.radio("Выберите ресурс:", resources, horizontal=False)

# Получение списка дат
dates = [datetime.strptime(data[0], DATE_FORMAT) for data in request_sql(
    conn, f"SELECT DISTINCT prices.date FROM prices LEFT JOIN cards ON prices.id = cards.id WHERE cards.type = '{type}' AND cards.resource = '{resource}'"
)]

# Проверка наличия дат
if not dates:
    st.error("Нет доступных данных для выбранного ресурса.")
    st.stop()

st.write(f"Загруженные даты: {dates}")

# График изменения цен
st.markdown(HEADER_FORMAT.format(f'Динамика изменения цен. Ресурс: {resource}'), unsafe_allow_html=True)

dates.sort()
dates = [date.strftime(DATE_FORMAT) for date in dates]

if len(dates) > 1:
    start_date, end_date = st.select_slider("Выберите даты", options=dates, value=[dates[0], dates[-1]])
    choose_dates = dates[dates.index(start_date):dates.index(end_date) + 1]
else:
    st.warning("Недостаточно данных для выбора диапазона дат.")
    choose_dates = dates

query = f"""
    SELECT cards.category, prices.date,
    ROUND((SUM(prices.price)/SUM(analogues.price)-1)*100)
    FROM prices
    LEFT JOIN cards ON cards.id = prices.id
    LEFT JOIN analogues ON cards.name_analogue = analogues.name_analogue
    WHERE date in ({get_sql_list(choose_dates)})
    AND cards.type = '{type}'
    AND cards.resource = '{resource}'
    AND cards.name_analogue <> 0
...
data = pd.DataFrame(
    [[el[0], datetime.strptime(el[1], DATE_FORMAT), el[2]] for el in request_sql(conn, query)],
    columns=['Категория', 'Дата', '%']
)
...
# Изменение цен по позициям
st.markdown(HEADER_FORMAT.format(f'Цены {resource} с {choose_dates[0]} по {choose_dates[-1]}'), unsafe_allow_html=True)
categories = data["Категория"].unique()
category = st.radio("Выберите категорию:", categories, horizontal=True)

query = f"""
    SELECT analogues.name_analogue, prices.date,
    CAST(analogues.price AS INT),
    CAST(prices.price AS INT)
    FROM prices
    LEFT JOIN cards ON cards.id = prices.id
...
data = pd.DataFrame(request_sql(conn, query), columns=['Аналог', 'Дата', 'Цена аналога', 'Цена'])
data['Дата'] = data['Дата'].apply(lambda x: datetime.strptime(x, DATE_FORMAT))
data = pd.pivot_table(
    data=data,
    values='Цена',
    index=['Аналог', 'Цена аналога'],
...
# Создание списков для форматирования
red_cells = []
green_cells = []
for i in range(len(data)):
    for j in range(1, data.shape[1]):
...
data = data.style.apply(cell_styling, red=red_cells, green=green_cells, axis=None)
st.dataframe(data)
